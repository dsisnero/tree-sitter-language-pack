...
// Content-Hash: blake3:55763c024f2adf05101a8d29929a985bb7a57a7fc549b94f3b12c196bef6cbe2
// Source-Hash: blake3:c1ccf9dcd8fbf8d63cf598e44b01d693d1fcfebe02d99b75481eddfda4278f8f
// Schema-Version: v1
...
const parseFormat = schema.enum([ "sexp", "json" ])
                        .default("sexp")
...
function runCli(args, context) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("error", (error) => {
      if (error.code === "ENOENT") {
        resolve({
          ...
          metadata :
              {exitCode : 127, command : "ts-pack", subcommand : args[0]},
        });
        return;
      }
      reject(error);
    ...
    child.on("close", (exitCode, signal) => {
      ...
      resolve({
        title : exitCode === 0 ? `ts-pack ${args[0]}`
                               : `ts-pack ${args[0]} failed`,
  ...
  tool : {
    tspack_parse : tool({
      ...
      args : {
        ...
        language : schema.string().min(1).optional().describe(
    ...
    tspack_process : tool({
      ...
      args : {
        ...
        language : schema.string().min(1).optional().describe(
            "Language override (auto-detected from extension if omitted)."),
        all : schema.boolean().optional().describe(
            "Enable all analysis features."),
        structure : schema.boolean().optional().describe(
        ...
        docstrings :
            schema.boolean().optional().describe("Extract docstrings."),
        diagnostics :
            schema.boolean().optional().describe("Include syntax diagnostics."),
        chunk_size : schema.number().int().positive().optional().describe(
    ...
    tspack_info : tool({
      ...
      args : {
        language : schema.string().min(1).describe(
            "Language name (e.g. python, rust, typescript)."),
      },
      async execute(
          args, context) { return runCli([ "info", args.language ], context);},
    }),
  },
});
