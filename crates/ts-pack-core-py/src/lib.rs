...
impl DataAttribute {
    ...
    #[must_use]
    #[pyo3(signature = (name=Self::default().name, value=Self::default().value, span=None))]
    #[new]
    pub fn new(name: String, value: String, span: Option<Span>) -> Self {
        Self {
            name,
            value,
...
impl DataNode {
    #[allow(clippy::too_many_arguments)]
    #[must_use]
    #[pyo3(signature = (kind=Self::default().kind, attributes=Self::default().attributes, children=Self::default().children, span=None, key=None, value=None))]
    #[new]
    pub fn new(
        kind: DataNodeKind,
...
impl ProcessConfig {
    #[allow(clippy::too_many_arguments)]
    #[must_use]
    #[pyo3(signature = (language=Self::default().language, structure=Self::default().structure, imports=Self::default().imports, exports=Self::default().exports, comments=Self::default().comments, docstrings=Self::default().docstrings, symbols=Self::default().symbols, diagnostics=Self::default().diagnostics, data_extraction=Self::default().data_extraction, chunk_max_size=None))]
    #[new]
    pub fn new(
        language: String,
        structure: bool,
...
impl DataNodeKind {
    ...
    fn __new__(value: pyo3::Bound<'_, pyo3::PyAny>) -> pyo3::PyResult<Self> {
        ...
        let type_name = stringify!(DataNodeKind);
    ...
    fn __str__(&self) -> PyResult<String> {
        serde_json::to_value(self)
            ...
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(format!("Failed to serialize DataNodeKind: {e}")))
    }
...
#[pyfunction]
#[pyo3(signature = (ext))]
pub fn detect_language_from_extension(py: Python<'_>, ext: String) -> Option<String> {
    py.detach(|| tree_sitter_language_pack::detect_language_from_extension(&ext))
        .map(Into::into)
}

#[pyfunction]
#[pyo3(signature = (path))]
pub fn detect_language_from_path(py: Python<'_>, path: String) -> Option<String> {
    py.detach(|| tree_sitter_language_pack::detect_language_from_path(&path))
        .map(Into::into)
}

#[pyfunction]
#[pyo3(signature = (content))]
pub fn detect_language_from_content(py: Python<'_>, content: String) -> Option<String> {
    py.detach(|| tree_sitter_language_pack::detect_language_from_content(&content))
        .map(Into::into)
}

#[pyfunction]
#[pyo3(signature = (language))]
pub fn get_highlights_query(py: Python<'_>, language: String) -> Option<String> {
    py.detach(|| tree_sitter_language_pack::get_highlights_query(&language))
        .map(Into::into)
}

#[pyfunction]
#[pyo3(signature = (language))]
pub fn get_injections_query(py: Python<'_>, language: String) -> Option<String> {
    py.detach(|| tree_sitter_language_pack::get_injections_query(&language))
        .map(Into::into)
}

#[pyfunction]
#[pyo3(signature = (language))]
pub fn get_locals_query(py: Python<'_>, language: String) -> Option<String> {
    py.detach(|| tree_sitter_language_pack::get_locals_query(&language))
        .map(Into::into)
}

#[pyfunction]
#[pyo3(signature = (language))]
pub fn get_tags_query(py: Python<'_>, language: String) -> Option<String> {
    py.detach(|| tree_sitter_language_pack::get_tags_query(&language))
        .map(Into::into)
}

#[pyfunction]
#[pyo3(signature = (language))]
pub fn get_indents_query(py: Python<'_>, language: String) -> Option<String> {
    py.detach(|| tree_sitter_language_pack::get_indents_query(&language))
        .map(Into::into)
}

#[pyfunction]
#[pyo3(signature = (language))]
pub fn get_folds_query(py: Python<'_>, language: String) -> Option<String> {
    py.detach(|| tree_sitter_language_pack::get_folds_query(&language))
        .map(Into::into)
}
...
#[pyo3::prelude::pyfunction]
pub fn get_parser(py: pyo3::Python<'_>, name: String) -> pyo3::PyResult<pyo3::Py<pyo3::PyAny>> {
    // Construct tree_sitter.Parser via Python-side factory.
    let _dep = get_language(py, name)?.bind(py).clone();
    let _ts_mod = py.import("tree_sitter")?;
    let _cls = _ts_mod.getattr("Parser")?;
    Ok(_cls.call1((_dep,))?.unbind())
}

#[pyfunction]
#[pyo3(signature = (path))]
pub fn detect_language(py: Python<'_>, path: String) -> Option<String> {
    py.detach(|| tree_sitter_language_pack::detect_language(&path))
        .map(Into::into)
}
...
#[pyfunction]
#[pyo3(signature = (name))]
pub fn has_language(py: Python<'_>, name: String) -> bool {
    py.detach(|| tree_sitter_language_pack::has_language(&name))
}
...
#[pyfunction]
#[pyo3(signature = (source, config))]
pub fn process(py: Python<'_>, source: String, config: ProcessConfig) -> PyResult<ProcessResult> {
    let config_core: tree_sitter_language_pack::ProcessConfig = config.into();

    py.detach(|| tree_sitter_language_pack::process(&source, &config_core))
        .map(Into::into)
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
pub fn init(py: Python<'_>, config: PackConfig) -> PyResult<()> {
    let config_core: tree_sitter_language_pack::PackConfig = config.into();

    py.detach(|| tree_sitter_language_pack::init(&config_core))
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
#[pyfunction]
#[pyo3(signature = (config))]
pub fn configure(py: Python<'_>, config: PackConfig) -> PyResult<()> {
    let config_core: tree_sitter_language_pack::PackConfig = config.into();

    py.detach(|| tree_sitter_language_pack::configure(&config_core))
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
pub fn download(py: Python<'_>, names: Vec<String>) -> PyResult<usize> {
    let names_refs: Vec<&str> = names.iter().map(|s| s.as_str()).collect();

    py.detach(|| tree_sitter_language_pack::download(&names_refs))
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
pub fn prefetch(py: Python<'_>, languages: Vec<String>) -> PyResult<()> {
    let languages_refs: Vec<&str> = languages.iter().map(|s| s.as_str()).collect();

    py.detach(|| tree_sitter_language_pack::prefetch(&languages_refs))
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
#[pyfunction]
#[pyo3(signature = (name))]
pub fn download_group(py: Python<'_>, name: String) -> PyResult<usize> {
    py.detach(|| tree_sitter_language_pack::download_group(&name))
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
pyo3::create_exception!(_native, LanguageNotFoundError, Error);
pyo3::create_exception!(_native, DynamicLoadError, Error);
pyo3::create_exception!(_native, NullLanguagePointerError, Error);
pyo3::create_exception!(_native, ParserSetupError, Error);
pyo3::create_exception!(_native, LockPoisonedError, Error);
pyo3::create_exception!(_native, ConfigError, Error);
pyo3::create_exception!(_native, ParseFailedError, Error);
pyo3::create_exception!(_native, QueryError, Error);
pyo3::create_exception!(_native, InvalidRangeError, Error);
pyo3::create_exception!(_native, DownloadError, Error);
pyo3::create_exception!(_native, ChecksumMismatchError, Error);
pyo3::create_exception!(_native, CacheLockError, Error);
...
pub fn _native(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(get_indents_query, m)?)?;
    m.add_function(wrap_pyfunction!(get_folds_query, m)?)?;
