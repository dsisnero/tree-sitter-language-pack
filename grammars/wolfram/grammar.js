...
const grammar0 = (/** @type any */ e) => {
  return grammar({
    ...
    rules : Object.fromEntries(
        Object.entries(e.rules).filter(([, v ]) => v !== EXTERNAL)),
  });
};
...
const optional0 =
    (/** @type any */ rule) => { return optional(globalReplacer(rule)); };
...
const tmp5 = (() => {
  ...
  const tmp = (/** @type any */ rules) => {
    ...
    const op_names = keys.filter(
        (e) => !e.endsWith("_operator") && !e.endsWith("_operator_left") &&
               !e.endsWith("_operator_right"),
    );
    return {
      ...Object.fromEntries(
      ...
      _expr_op_top : (/** @type any */ $) =>
          choice(...op_names.map((e) => $[`${e}_top`])),
      _expr_op : (/** @type any */ $) =>
          prec(35, choice(...op_names.map((e) => $[e]))),
    };
  };
  return tmp;
})();
...
module.exports = grammar0({
  ...
  conflicts : ($) => [
                ...
                [
                  $.underscript_top, $.underoverscript_top,
                  $.overunderscript_top
                ],
                ...
                [
                  $.tag_set_operator_left, $.tag_set_delayed_operator_left,
  ...
  rules : {
    ...
    _expr : ($) =>
              prec(20, choice(prec.left(seq($._expr1_unary_special,
                                            repeat1($._expr1_unary_special))),
                              $._expr1)),
    _expr1_symbol_top : ($) => prec(10, choice($._symbol)),
    _expr1_symbol : ($) => prec(10, seq(repeat(/\n+/), choice($._symbol))),
    _expr1_unary_top : ($) => prec(
                         20,
                         choice(
                             $._expr_finished,
                             //
    ...
    _expr1_unary_special : ($) => prec.left(
                             seq(
                                 repeat(/\n+/),
                                 choice(
                                     $._expr_finished,
