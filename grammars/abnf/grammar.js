num_val_template = (indicator, char_rule) => seq(
    indicator,
    repeat1(char_rule),
    optional(choice(repeat1(seq(".", repeat1(char_rule))),
...
module.exports = grammar({
  ...
  rules : {
    ...
    rulelist : ($) => repeat1(
                 choice($.rule, prec(1, seq(repeat($._c_wsp), $._c_nl)))),
    ...
    concatenation : ($) => seq($.repetition,
                               repeat(seq(repeat1($._c_wsp), $.repetition))),

    repetition : ($) => seq(optional($.repeat), $.element),

    repeat : ($) => choice(repeat1($.DIGIT),
                           seq(repeat($.DIGIT), "*", repeat($.DIGIT))),

    // NB: $.core_rulename isn't explicitly defined in RFC 5234.
    element : ($) => choice($.core_rulename, $.rulename, $.group, $.option,
    ...
    // Updated by RFC 7405
    char_val : ($) =>
                 choice($.case_insensitive_string, $.case_sensitive_string),
