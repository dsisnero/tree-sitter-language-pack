...
module.exports = grammar({
  ...
  rules : {
    ...
    object : ($) => seq(
               ...
               field("body",
                     seq("{", $.code, repeat(choice($.data, $.object)), "}")),
               ),

    code : ($) => seq("code", field("body", $.block)),
    data : ($) => seq("data", field("key", $.stringLiteral),
                      field("value", choice($.hexLiteral, $.stringLiteral))),
    ...
    functionDefinition : ($) => seq(
                           ...
                           ")",
                           optional(seq(
                               "->", field("returns", $.typedIdentifierList))),
                           ),
    variableDeclaration : ($) => seq(
                            "let", field("left", $.typedIdentifierList),
                            optional(seq(":=", field("right", $.expression)))),
    assignment : ($) => seq(field("left", $.identifierList),
    ...
    switch : ($) => seq(
               ...
               field("body", choice(seq(repeat1($.case), optional($.default)),
                                    $.default)),
               ),
    case : ($) => seq("case", field("value", $.literal), $.block),
    default : ($) => seq("default", $.block),
    forLoop : ($) => seq("for", $.block, $.expression, $.block, $.block),
    breakContinue : (_$) => choice("break", "continue"),
    leave : (_$) => "leave",
    functionCall : ($) => seq(field("name", $.identifier), "(",
    ...
    typeName : ($) => $.identifier,
    typedIdentifierList : ($) =>
                            seq($.identifier, optional(seq(":", $.typeName)),
                                repeat(seq(",", $.identifier,
                                           optional(seq(":", $.typeName))))),
    literal : ($) => seq(
                choice($.numberLiteral, $.stringLiteral, $.hexLiteral,
                       $.trueLiteral, $.falseLiteral),
                optional(seq(":", $.typeName)),
                ),
    numberLiteral : ($) => choice($.hexNumber, $.decimalNumber),
    hexLiteral : (_$) => /hex("([0-9a-fA-F]{2})*"|'([0-9a-fA-F]{2})*')/,
    stringLiteral : (_$) => /"([^"\r\n\\]|\\.)*"/,
    trueLiteral : (_$) => "true",
    falseLiteral : (_$) => "false",
    hexNumber : (_$) => /0x[0-9a-fA-F]+/,
    decimalNumber : (_$) => /[0-9]+/,
