module.exports = grammar({
  ...
  rules : {
    ...
    inst : ($) => choice("active", seq("active", "[", $.const_expr, "]"),
                         seq("active", "[", $.uname, "]")),
    body : ($) => seq("{", $.sequence, optional($._ms), "}"),
    sequence : ($) => sep1($.step, $._ms),
    arg : ($) => choice($.expr, seq($.expr, ",", $.arg)),
    ...
    rarg : ($) => choice($.varref, seq("eval", "(", $.expr, ")"), $.const,
                         seq("-", $.const)),
    rargs : ($) => prec.left(1, choice($.rarg, seq($.rarg, ",", $.rargs),
                                       seq($.rarg, "(", $.rargs, ")"),
                                       seq("(", $.rargs, ")"))),
    ...
    for_post : ($) =>
                 choice(seq("{", $.sequence, optional($._semi), "}"),
    ...
    oname : ($) => seq(":", $.uname),
    basetype : ($) => choice(seq($.type, optional($.oname)), $.uname),
