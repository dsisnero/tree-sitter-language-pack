...
private class StructureKindDeserializer : com.fasterxml.jackson.databind.deser.std.StdDeserializer<StructureKind>(StructureKind::class.java) {
    ...
    override fun deserialize(
        ...
        if (node.isTextual) {
            return when (node.asText()) {
                ...
                "Namespace" -> StructureKind.Namespace
                    ...
                    parser, "Unknown StructureKind unit variant", node.asText(), StructureKind::class.java,
                )
            }
        }
        if (node.isObject) {
            ...
            if (it.hasNext()) {
                ...
                if (!it.hasNext()) {
                    ...
                    return when (entry.key) {
                        "Other" -> StructureKind.Other(ctx.readTreeAsValue<String>(payload, String::class.java))
                        else -> throw com.fasterxml.jackson.databind.exc.InvalidFormatException(
                            parser, "Unknown StructureKind data variant", entry.key, StructureKind::class.java,
                        )
                    }
                }
            }
        ...
        throw com.fasterxml.jackson.databind.exc.InvalidFormatException(
            parser, "Cannot deserialize StructureKind: expected string or single-field object", null, StructureKind::class.java,
        )
    }
}

private class StructureKindSerializer : com.fasterxml.jackson.databind.set.std.StdSerializer<StructureKind>(StructureKind::class.java) {
    @Suppress("LongMethod")
