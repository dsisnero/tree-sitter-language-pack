...
android {
    namespace = "io.xberg.tslp.android"
    compileSdk = 36
...
dependencies {
    ...
    api("io.github.tree-sitter:ktreesitter:0.25.0")
...
tasks.register("copyHostJni", Copy::class) {
    if (project.properties["alef.skipHostJni"] != "true") {
        ...
        val jniCratePath = file("../../crates/tree-sitter-language-pack-jni")
...
tasks.register("validateJniLibsForRelease") {
    doFirst {
        ...
        if (releaseAssemble) {
            ...
            // lib*_ffi.so instead of libts_pack_jni.so) passes the emptiness
            // check above but fails at runtime with UnsatisfiedLinkError, because
            // System.loadLibrary("ts_pack_jni") needs the JNI entry points.
            val expectedJniLib = "libts_pack_jni.so"
            ...
            if (abisMissingJniLib.isNotEmpty()) {
                throw GradleException(
                    ...
                    "export the JNI symbols System.loadLibrary(\"ts_pack_jni\") requires and " +
...
mavenPublishing {
    ...
    pom {
        ...
        description.set("Pre-compiled tree-sitter grammars for 371 programming languages")
        ...
        scm {
            ...
            developerConnection.set("scm:git:ssh://git@github.com:xberg-io/tree-sitter-language-pack.git")
        }
    }
}
