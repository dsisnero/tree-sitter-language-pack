...
impl Parser {
    ...
    fn parse_bytes(&self, source: magnus::RString) -> Option<Tree> {
...
fn get_tags_query(language: String) -> Option<String> {
    tree_sitter_language_pack::get_tags_query(&language).map(Into::into)
}

fn get_indents_query(language: String) -> Option<String> {
    tree_sitter_language_pack::get_indents_query(&language).map(Into::into)
}

fn get_folds_query(language: String) -> Option<String> {
...
fn prefetch(languages: Vec<String>) -> Result<(), Error> {
    let languages_refs: Vec<&str> = languages.iter().map(|s| s.as_str()).collect();
    let result = tree_sitter_language_pack::prefetch(&languages_refs).map_err(|e| {
        magnus::Error::new(
            unsafe { Ruby::get_unchecked() }.exception_runtime_error(),
            e.to_string(),
        )
...
fn ruby_init(ruby: &Ruby) -> Result<(), Error> {
    ...
    module.define_module_function("get_tags_query", function!(get_tags_query, 1))?;

    module.define_module_function("get_indents_query", function!(get_indents_query, 1))?;

    module.define_module_function("get_folds_query", function!(get_folds_query, 1))?;
    ...
    module.define_module_function("prefetch", function!(prefetch, 1))?;
