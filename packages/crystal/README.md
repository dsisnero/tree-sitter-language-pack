...
Crystal bindings for **tree-sitter-language-pack** — 371 pre-compiled tree-sitter
grammars and code-intelligence primitives. Idiomatic Crystal API over the C FFI
(`libts_pack_core_ffi`). Distributed via the `dsisnero/tree-sitter-language-pack.cr`
shard; the native library is downloaded on install via `download_ffi.sh`.
...
See the [language guide](https://docs.tree-sitter-language-pack.xberg.io) for `crystal`-specific usage.
