...
For advanced users who prefer manual MCP registration over the plugin, the CLI exposes a raw MCP server. See the [MCP Server guide](/guides/mcp-server/) for stdio and HTTP transport setup with any compatible IDE.
