...
## PHP API Reference <span class="version-badge">v1.12.5</span>
...
```php
$result = process("value", new ProcessConfig());
```
...
```php
public static function prefetch(array<string> $languages): void
```
...
```php
$result = $instance->downloadAllBestEffort();
```
...
```php
$result = $instance->getLanguage("value");
```
...
```php
$result = $instance->process("value", new ProcessConfig());
```
...
```php
$result = $instance->parseBytes("data");
```
