...
## Swift API Reference <span class="version-badge">v1.13.6</span>
...
```swift
let result = detectLanguageFromExtension("value")
```
...
```swift
let result = detectLanguageFromPath("value")
```
...
```swift
let result = detectLanguageFromContent("value")
```
...
```swift
let result = getHighlightsQuery("value")
```
...
```swift
let result = getInjectionsQuery("value")
```
...
```swift
let result = getLocalsQuery("value")
```
...
```swift
public static func getTagsQuery(language: String) -> String?
```

**Example:**

```swift
let result = getTagsQuery("value")
```
...
```swift
public static func getIndentsQuery(language: String) -> String?
```

**Example:**

```swift
let result = getIndentsQuery("value")
```
...
```swift
public static func getFoldsQuery(language: String) -> String?
```

**Example:**

```swift
let result = getFoldsQuery("value")
```
...
```swift
let result = try getLanguage("value")
```
...
```swift
let result = try getParser("value")
```
...
```swift
let result = detectLanguage("value")
```
...
```swift
let result = try process("value", ProcessConfig())
```
...
```swift
public static func prefetch(languages: [String]) throws
```
...
```swift
let result = try downloadGroup("value")
```
...
```swift
let result = downloadedLanguages()
```
...
| `chunkIndex` | `UInt64` | — | Zero-indexed position of this chunk within the file's chunk list. |
| `totalChunks` | `UInt64` | — | Total number of chunks the file was split into. |
| `nodeTypes` | `\[String\]` | `\[\]` | Tree-sitter node kinds that appear at the top level of this chunk. |
| `contextPath` | `\[String\]` | `\[\]` | Hierarchical path of enclosing structural items (e.g., `\["MyClass", "my_method"\]`). |
| `symbolsDefined` | `\[String\]` | `\[\]` | Names of symbols defined within this chunk. |
| `comments` | `\[CommentInfo\]` | `\[\]` | Comments contained within this chunk. |
| `docstrings` | `\[DocstringInfo\]` | `\[\]` | Docstrings contained within this chunk. |
...
| `startByte` | `UInt64` | — | Inclusive start byte offset of this chunk in the original source. |
| `endByte` | `UInt64` | — | Exclusive end byte offset of this chunk in the original source. |
| `startLine` | `UInt64` | — | Zero-indexed start line of this chunk. |
| `endLine` | `UInt64` | — | Zero-indexed end line of this chunk. |
...
| `attributes` | `\[DataAttribute\]` | `\[\]` | Attributes on element-shape nodes (XML `STag` attributes). Empty for all other kinds. |
| `children` | `\[DataNode\]` | `\[\]` | Children for nested containers and XML element bodies. |
...
| `parsedSections` | `\[DocSection\]` | `\[\]` | Parsed sections of the docstring (Args, Returns, Raises, etc.). |
...
```swift
let result = try DownloadManager.new("value")
```
...
```swift
let result = instance.installedLanguages()
```
...
```swift
let result = try instance.downloadAllBestEffort()
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `totalLines` | `UInt64` | — | Total number of lines (including blank and comment lines). |
| `codeLines` | `UInt64` | — | Number of lines containing non-blank, non-comment source code. |
| `commentLines` | `UInt64` | — | Number of lines that are entirely comments. |
| `blankLines` | `UInt64` | — | Number of blank (whitespace-only) lines. |
| `totalBytes` | `UInt64` | — | Total byte length of the source file. |
| `nodeCount` | `UInt64` | — | Total number of nodes in the syntax tree. |
| `errorCount` | `UInt64` | — | Number of error nodes in the syntax tree (parse errors). |
| `maxDepth` | `UInt64` | — | Maximum nesting depth reached in the syntax tree. |
...
| `items` | `\[String\]` | `\[\]` | Specific names imported from the source module. |
...
```swift
public static func new() -> LanguageRegistry
```
...
```swift
let result = try instance.getLanguage("value")
```
...
```swift
let result = instance.availableLanguages()
```
...
```swift
public func hasParser(name: String) -> Bool
```

**Example:**

```swift
let result = instance.hasParser("value")
```
...
```swift
let result = instance.hasLanguage("value")
```
...
```swift
let result = instance.languageCount()
```
...
```swift
let result = try instance.process("value", ProcessConfig())
```
...
```swift
let result = instance.startPosition()
```
...
```swift
let result = instance.endPosition()
```
...
```swift
let result = instance.childCount()
```
...
```swift
let result = instance.namedChild(42)
```
...
```swift
let result = instance.namedChildCount()
```
...
```swift
let result = instance.childByFieldName("value")
```
...
```swift
let result = instance.parseBytes(Data("data".utf8))
```
...
```swift
let result = instance.withChunking(42)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `maxSize` | `UInt64` | Yes | The max size |
...
```swift
let result = instance.withDataExtraction(true)
```
...
| `structure` | `\[StructureItem\]` | `\[\]` | Top-level structural items (functions, classes, etc.). |
| `imports` | `\[ImportInfo\]` | `\[\]` | Import statements extracted from the source. |
| `exports` | `\[ExportInfo\]` | `\[\]` | Export statements extracted from the source. |
| `comments` | `\[CommentInfo\]` | `\[\]` | Comments extracted from the source. |
| `docstrings` | `\[DocstringInfo\]` | `\[\]` | Docstrings extracted from the source. |
| `symbols` | `\[SymbolInfo\]` | `\[\]` | Symbol definitions (variables, types, functions) extracted from the source. |
| `diagnostics` | `\[Diagnostic\]` | `\[\]` | Parse diagnostics (syntax errors, missing nodes) from tree-sitter. |
| `chunks` | `\[CodeChunk\]` | `\[\]` | Syntax-aware code chunks produced when chunking is enabled. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `startByte` | `UInt64` | — | Inclusive start byte offset in the source. |
| `endByte` | `UInt64` | — | Exclusive end byte offset in the source. |
| `startLine` | `UInt64` | — | Zero-indexed line number of the span's start. |
| `startColumn` | `UInt64` | — | Zero-indexed column number of the span's start. |
| `endLine` | `UInt64` | — | Zero-indexed line number of the span's end. |
| `endColumn` | `UInt64` | — | Zero-indexed column number of the span's end. |
...
| `children` | `\[StructureItem\]` | `\[\]` | Nested structural items (e.g., methods within a class). |
| `decorators` | `\[String\]` | `\[\]` | Decorator or attribute names applied to the item. |
...
```swift
let result = instance.gotoFirstChild()
```
...
```swift
let result = instance.gotoParent()
```
...
```swift
let result = instance.gotoNextSibling()
```
