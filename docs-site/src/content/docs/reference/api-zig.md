...
## Zig API Reference <span class="version-badge">v1.13.0</span>
...
```zig
pub fn get_indents_query(language: [:0]const u8) ?[:0]const u8
```
...
```zig
pub fn get_folds_query(language: [:0]const u8) ?[:0]const u8
```
...
```zig
pub fn prefetch(languages: []const [:0]const u8) Error!void
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `languages` | `\[\]const \[:0\]const u8` | Yes | The languages |
