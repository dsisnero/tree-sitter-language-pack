...
## Ruby API Reference <span class="version-badge">v1.13.2</span>
...
```ruby
def self.get_indents_query(language)
```
...
```ruby
def self.get_folds_query(language)
```
