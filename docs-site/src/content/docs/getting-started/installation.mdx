...
console.log(tslp.languageCount()); // 371
```
...
```rust
fn main() {
    println!("{}", tree_sitter_language_pack::language_count()); // 371
}
```
...
func main() {
    fmt.Println(tslp.LanguageCount()) // 371
}
```
...
```kotlin
dependencies {
    implementation("io.xberg.treesitterlanguagepack:tree-sitter-language-pack:1.12.5")
}
```
...
public class Main {
    public static void main(String[] args) {
        System.out.println(TreeSitterLanguagePack.languageCount()); // 371
    }
}
```
...
Console.WriteLine(TreeSitterLanguagePackConverter.LanguageCount()); // 371
```
...
puts TreeSitterLanguagePack.language_count # 371
```
...
```elixir
IO.puts TreeSitterLanguagePack.language_count() # 371
```
...
```php
<?php
echo \ts_pack_language_count(); // 371
```
